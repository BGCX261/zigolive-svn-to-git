...
class HTML_AJAX_Server 
{
     ...
     */
    function HTML_AJAX_Server($request=null,$serverUrl = false) 
    {
    ...
    function clientJsLocation() 
    {
        if (!$this->clientJsLocation) {
            $path = 'C:\tim_jboss\seam_workbench\BarBottin\lib/PEAR/data'.DIRECTORY_SEPARATOR.'HTML_AJAX'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR;
    ...
    function _loadOptions() 
    {
        ...
        if (count($this->request->args)>0) {
            //$clients = explode(',',$this->ajax->_getVar('client'));
            $clients = $this->request->args;
        }else {
        	$clients = array("all");
        }
        $client = array();
